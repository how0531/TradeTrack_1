...
import { ChevronLeft, ChevronRight, Calculator, CalendarDays, Edit2, Trash2, ArrowUpCircle, ArrowDownCircle, BrainCircuit, ImageIcon, Scroll, PenTool, Globe, FileText, Download, Upload, ShieldAlert, Plus, X, AlertCircle, Briefcase, User, LogOut, Palette } from 'lucide-react';
...
export const CalendarView = ({ dailyPnlMap, currentMonth, setCurrentMonth, onDateClick, monthlyStats, hideAmounts, lang }: CalendarViewProps) => {
    ...
    const getHeatmapStyle = (pnl: number, day: string | number) => {
        ...
        const opacity = 0.15 + (intensity * 0.75); // Min 0.15 opacity

        if (pnl > 0) {
            // Profit: Red
            return { 
                bg: `rgba(208, 90, 90, ${opacity})`, 
                border: `1px solid rgba(208, 90, 90, ${Math.max(opacity, 0.3)})`, 
                text: opacity > 0.6 ? '#FFF' : THEME.RED 
            };
        } else {
            // Loss: Green (Asian Style - as requested)
            return { 
                bg: `rgba(91, 154, 139, ${opacity})`, 
                border: `1px solid rgba(91, 154, 139, ${Math.max(opacity, 0.3)})`, 
                text: opacity > 0.6 ? '#FFF' : THEME.GREEN 
            };
        }
    };
                    ...
                    {calendarDays.map((item) => {
                        ...
                        return (
                            <div key={item.key} onClick={() => { if (item.day && onDateClick) onDateClick(item.key); }} className={`aspect-square flex flex-col justify-center items-center text-center rounded-lg transition-all duration-200 ${item.day ? 'hover:brightness-110 cursor-pointer active:scale-95' : ''}`} style={{ backgroundColor: style.bg, border: style.border }}>
                                ...
                                {item.pnl !== 0 && item.day && (<div className={`text-[8px] font-bold mt-0.5 font-barlow-numeric ${Math.abs(item.pnl)/maxAbsPnl > 0.6 ? 'opacity-100' : 'opacity-80'}`} style={{ color: Math.abs(item.pnl)/maxAbsPnl > 0.6 ? '#FFF' : style.text }}>{hideAmounts ? '***' : (Math.abs(item.pnl) >= 1000 ? (Math.abs(item.pnl)/1000).toFixed(1) : Math.abs(item.pnl))}</div>)}
                            </div>
                        );
                    })}
                </div>
...
// --- LOG VIEW (DEEP MATTE STYLE) ---

const DeepMatteTradeItem = ({ trade, onEdit, onDelete, lang, hideAmounts, lossColor }: any) => {
    ...
    const indicatorColor = isProfit ? '#F87171' : (lossColor || '#CBD5E1'); // Red or Custom Loss

    return (
        <div className="relative mb-3 select-none">
            {/* Swipe Background */}
            <div className="absolute inset-0 rounded-xl flex items-center justify-between px-4 bg-[#0B0C10]">
                 <div className={`flex items-center gap-2 font-bold text-xs transition-opacity`} style={{color: THEME.BLUE, opacity: currentX > 20 ? 1 : 0}}>
                    <Edit2 size={16} /> {t.editTrade}
                </div>
                <div className={`flex items-center gap-2 font-bold text-xs transition-opacity`} style={{color: THEME.RED, opacity: currentX < -20 ? 1 : 0}}>
                    {t.delete} <Trash2 size={16} />
                </div>
            </div>

            {/* Deep Matte Card */}
            <div 
                className="relative p-3.5 rounded-xl transition-transform duration-200 ease-out active:scale-[0.98] flex justify-between items-center"
                style={{ 
                    backgroundColor: '#16181C',
                    boxShadow: '0 1px 0 0 rgba(255,255,255,0.08) inset, 0 4px 10px rgba(0,0,0,0.5)',
                    transform: `translateX(${currentX}px)`,
                }}
                onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd} onClick={() => onEdit(trade)}
            >
                {/* Left: Info */}
                <div className="flex flex-col justify-center gap-1 min-w-0 pr-4">
                    {trade.strategy ? (
                        <span className="text-sm font-bold text-white leading-tight truncate">{trade.strategy}</span>
                    ) : null}
                    
                    <div className="flex items-center gap-2">
                        {trade.emotion && (
                            <span className="text-[10px] font-medium text-slate-500 bg-[#0B0C10] border border-white/5 px-1.5 py-0.5 rounded">
                                {trade.emotion}
                            </span>
                        )}
                        {(trade.note || trade.image) && (
                            <FileText size={12} className="text-slate-600" />
                        )}
                         {!trade.strategy && !trade.emotion && !trade.note && !trade.image && (
                            <span className="text-[10px] text-slate-600 italic">No details</span>
                        )}
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                    <div className="text-lg font-barlow-numeric font-medium text-slate-200 tracking-tight">
...
export const LogsView = ({ trades, lang, hideAmounts, onEdit, onDelete, lossColor }: LogsViewProps) => {
    const t = I18N[lang] || I18N['zh'];
    ...
    const getItemHeight = (item: any) => {
        if (item.type === 'header') return 48; // Compact header
        return 76; // Card height (66px) + Gap (10px)
    };
            ...
            <VirtualList 
                ...
                renderItem={(item, index) => {
                    if (item.type === 'header') {
                        ...
                        // Day Header: Sticky glass effect
                        return (
                            <div className="sticky top-0 z-20 px-1 py-2 flex items-center justify-between backdrop-blur-xl bg-[#0B0C10]/60 border-b border-white/5 mb-2">
                                <span className="text-xs font-bold text-slate-400 pl-1">{dayStr}</span>
                                <div className="flex items-center gap-2">
                                    <span className="text-[10px] font-bold text-slate-600 bg-white/5 px-1.5 rounded">{item.count} {t.trades}</span>
                                    <span className="text-xs font-bold font-barlow-numeric" style={{ color: item.pnl >= 0 ? '#E2E8F0' : (lossColor || '#94A3B8') }}>
    ...
    return (
        <div className="space-y-4 pb-24">
             ...
             <div className="p-5 rounded-xl space-y-5 bg-[#141619] border border-white/5 shadow-xl">
                 ...
                 {/* APPEARANCE */}
                 <div>
                    <h3 className="text-[10px] font-bold uppercase text-slate-500 mb-3 flex items-center gap-1"><Palette size={12} /> {t.appearance}</h3>
                    <div className="flex items-center justify-between p-3 rounded-lg bg-[#1C1E22] border border-white/5">
                        <span className="text-xs font-bold text-slate-300">{t.lossColor}</span>
                        <div className="flex items-center gap-2">
                            <input 
                                type="color" 
                                value={lossColor || THEME.LOSS_WHITE} 
                                onChange={(e) => setLossColor(e.target.value)} 
                                className="w-8 h-8 rounded cursor-pointer border-none bg-transparent" 
                            />
                            <span className="text-[10px] font-mono text-slate-500">{lossColor || THEME.LOSS_WHITE}</span>
                        </div>
                    </div>
                 </div>
                 <div className="w-full h-[1px] bg-white/5"></div>
                 ...
                 <div>
                    ...
                    <div className="flex gap-3">
                        ...
                        <input type="file" accept=".csv" ref={fileRef} style={{ display: 'none' }} onChange={(e: any) => actions.handleImportCSV(e, t)} />
                    </div>
                 </div>
                 <div className="w-full h-[1px] bg-white/5"></div>
                 <div>
