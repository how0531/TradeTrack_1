                        ...
                        <div key={p.id} className="p-3 bg-[#0B0C10] rounded-xl border border-white/5 flex flex-col gap-3">
                            <div className="flex justify-between items-center mb-1">
                                <div className="flex-1 mr-4">
                                    <span className="text-[8px] text-slate-500 uppercase font-bold block mb-1">{t.portfolioName}</span>
                                    <input 
                                        type="text" 
                                        value={p.name} 
                                        onChange={(e) => actions.updatePortfolio(p.id, 'name', e.target.value)}
                                        className="w-full bg-transparent text-sm font-bold text-white outline-none border-b border-white/10 focus:border-gold/50 py-1 transition-colors"
                                        placeholder={t.portfolioName}
                                    />
                                </div>
                                {portfolios.length > 1 && (
                                    <button onClick={() => actions.updateSettings('portfolios', portfolios.filter((x: Portfolio) => x.id !== p.id))} className="p-2 bg-white/5 rounded-lg text-slate-500 hover:text-red-400 hover:bg-white/10 transition-all">
                                        <Trash2 size={14} />
                                    </button>
                                )}
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <span className="text-[8px] text-slate-500 uppercase font-bold block mb-1">{t.initialCapital}</span>
                                ...
                                <div>
                                    <span className="text-[8px] text-slate-500 uppercase font-bold block mb-1">{lang === 'zh' ? '代表色' : 'Colors'}</span>
                                    <div className="flex gap-2">
                                        <div className="flex-1 bg-white/5 rounded flex items-center justify-between px-2 py-1 border border-white/5">
                                            <span className="text-[9px] text-slate-400">{t.profit}</span>
                                            <ColorPicker value={p.profitColor} onChange={(c) => actions.updatePortfolio(p.id, 'profitColor', c)} />
                                        </div>
                                        <div className="flex-1 bg-white/5 rounded flex items-center justify-between px-2 py-1 border border-white/5">
                                            <span className="text-[9px] text-slate-400">{t.loss}</span>
                                            <ColorPicker value={p.lossColor} onChange={(c) => actions.updatePortfolio(p.id, 'lossColor', c)} />
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="text-right">
                                <span className="text-[9px] text-slate-800 font-mono tracking-widest">ID: {p.id.slice(-6)}</span>
                            </div>
                        </div>
                    ))}
                </div>
...
const AppearanceSection = ({ lang, setLang, strategies, emotions, actions, t }: any) => {
...
const DataManagementSection = ({ trades, actions, t }: any) => {
    ...
    return (
        <div className="pb-24 px-1 max-w-md mx-auto">
            ...
            <AppearanceSection 
                ...
                t={t} 
            />
            <DataManagementSection trades={trades} actions={actions} t={t} />
            
            <div className="text-center pt-4 pb-8">
                <p className="text-[9px] text-slate-800 font-mono tracking-widest uppercase">TradeTrack Pro v2.6.0</p>
            </div>
        </div>
    );
};
